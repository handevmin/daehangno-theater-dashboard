...
function kopisDevProxy(mode: string) {
  return {
    ...
    async configureServer(server: any) {
      ...
      const { default: handleQuizStat } = await import('./api/quiz-stat.js')
      const { default: handleSaveQuiz } = await import('./api/save-quiz.js')
      const { default: handleSeenStat } = await import('./api/seen-stat.js')
      ...
      server.middlewares.use('/api/quiz-stat', (req: any, res: any) => handleQuizStat(req, res))
      server.middlewares.use('/api/save-quiz', (req: any, res: any) => handleSaveQuiz(req, res))
      server.middlewares.use('/api/seen-stat', (req: any, res: any) => handleSeenStat(req, res))
